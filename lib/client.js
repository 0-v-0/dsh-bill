...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DICT_ZH = {
      ...
      'storage.title': '存储与导入',
      'storage.desc': '内存环形缓冲的容量,以及启动时历史导入的总预算。',
      'storage.maxRecords': '记录上限',
      'storage.maxRecords.desc': '内存环形缓冲条数,范围 10–1,000,000;超出后最旧记录折叠为汇总。',
      'storage.backfillTimeoutMs': '历史导入预算 (ms)',
      'storage.backfillTimeoutMs.desc': '启动时导入尚无记录会话的总时限;0 表示完全禁用导入。',
      'rebuild.title': '从日志重建',
      'rebuild.desc': '从会话日志全量重建记录与汇总,清除 fork/subagent 继承前缀导致的重复计费。',
      'rebuild.button': '重建日志',
      'rebuild.button.desc': '备份后清空,再按磁盘上仍保留的会话日志重新导入;已删除的会话日志无法恢复。',
      'rebuild.modal.title': '从日志重建',
      'rebuild.modal.desc': '将先备份 records.jsonl 与 rollup.json,然后清空内存与文件,再从磁盘上仍保留的会话日志全量重新导入。已删除的会话日志无法恢复;导入中途超时则自动回滚、不改数据。',
      'rebuild.modal.timeout': '超时 (毫秒)',
      'rebuild.modal.timeout.desc': '导入总时限,超时则回滚。',
      'rebuild.modal.concurrency': '线程数',
      'rebuild.modal.concurrency.desc': '并行读取会话日志的线程数,默认本机线程数减一。',
      'rebuild.modal.confirm': '重建',
      'rebuild.modal.cancel': '取消',
      'rebuild.running': '重建中…',
      'rebuild.done': '完成:共 %n 条记录,总计 $%u',
      'rebuild.failed': '失败: ',
      'turn.cost': '本轮',
      'turn.steps': ' 步',
      'turn.cache': '缓存命中 ',
    ...
    var DICT_EN = {
      ...
      'storage.title': 'Storage & import',
      'storage.desc': 'In-memory ring size and the boot-time history import budget.',
      'storage.maxRecords': 'Max records',
      'storage.maxRecords.desc': 'In-memory ring size, 10–1,000,000; older entries are folded into a rollup.',
      'storage.backfillTimeoutMs': 'Import budget (ms)',
      'storage.backfillTimeoutMs.desc': 'Total budget for importing sessions without records at boot; 0 disables the import.',
      'rebuild.title': 'Rebuild from logs',
      'rebuild.desc': 'Rebuild records and the rollup from session logs, removing the inherited-prefix double-counts fork/subagent sessions caused.',
      'rebuild.button': 'Rebuild logs',
      'rebuild.button.desc': 'Backs up, clears, then re-imports every session from the logs still on disk; deleted sessions cannot be recovered.',
      'rebuild.modal.title': 'Rebuild from logs',
      'rebuild.modal.desc': 'Backs up records.jsonl and rollup.json, then clears memory and files and re-imports every session from the logs still retained on disk. Sessions whose logs were deleted cannot be recovered; rolls back automatically if the import times out, leaving data unchanged.',
      'rebuild.modal.timeout': 'Timeout (ms)',
      'rebuild.modal.timeout.desc': 'Total import time budget; aborts and rolls back on timeout.',
      'rebuild.modal.concurrency': 'Concurrency',
      'rebuild.modal.concurrency.desc': 'Parallel session-log readers; defaults to one less than the machine thread count.',
      'rebuild.modal.confirm': 'Rebuild',
      'rebuild.modal.cancel': 'Cancel',
      'rebuild.running': 'Rebuilding…',
      'rebuild.done': 'Done: %n records, total $%u',
    ...
    // Sizing knobs mirrored from the host: the same ranges it clamps into.
    var MIN_MAX_RECORDS = 10
    var MAX_MAX_RECORDS = 1000000
    var MAX_BACKFILL_TIMEOUT_MS = 2147483647
    var PREFS_DEFAULT = {
      ...
      maxRecords: 20000, backfillTimeoutMs: 60000,
    }
    ...
    /** A numeric preference: read through `usePrefs`, write clamped into range. */
    function useStorageNumber(key, min, max) {
      var value = usePrefs(function (p) { return p[key] })
      var write = function (next) {
        var n = Number(next)
        if (!Number.isFinite(n)) return
        var patch = {}
        patch[key] = Math.min(max, Math.max(min, Math.round(n)))
        prefsWrite(patch)
      }
      return [value, write]
    }
    ...
    function BillSettings(props) {
      ...
      return el('div', { style: { width: '100%', maxWidth: 720, boxSizing: 'border-box', minWidth: 0 } },
        ...
        el(StorageSection, { t: t }),
        el(RebuildSection, { t: t }))
    }
    ...
    /**
     * One numeric storage knob: label and hint left, number input right, the
     * same row layout as the surface toggles. The input keeps a local draft
     * while typing and commits — clamped into range by the host — on blur or
     * Enter, so an out-of-range value snaps back to the nearest valid one
     * instead of fighting the keystrokes.
     */
    function StorageRow(props) {
      ...
      var min = props.min
      var max = props.max
      var draft = React.useState(String(value))
      var text = draft[0]
      var setText = draft[1]
      React.useEffect(function () { setText(String(value)) }, [value])
      return el('div', { style: surfaceRow },
        el('div', { style: { minWidth: 0 } },
          el('div', { style: surfaceName }, t('storage.' + key)),
          el('div', { style: surfaceHint }, t('storage.' + key + '.desc'))),
        el('input', {
          type: 'number', min: min, max: max, step: 1,
          className: 'dshbill-field',
          value: text,
          onChange: function (e) { setText(e.target.value) },
          onBlur: function () { onCommit(text) },
          onKeyDown: function (e) { if (e.key === 'Enter') onCommit(text) },
          style: surfaceControl,
        }))
    }

    /**
     * Record storage: how many calls the in-memory ring keeps, and how long
     * the boot-time history import may run. Both are pure host-side sizing
     * knobs the settings page did not use to expose.
     */
    function StorageSection(props) {
      var t = props.t
      var maxPair = useStorageNumber('maxRecords', MIN_MAX_RECORDS, MAX_MAX_RECORDS)
      var budgetPair = useStorageNumber('backfillTimeoutMs', 0, MAX_BACKFILL_TIMEOUT_MS)
      return el('div', { style: section },
        el('div', { style: panelT }, t('storage.title')),
        el('div', { style: panelSub }, t('storage.desc')),
        el(StorageRow, {
          t: t, field: 'maxRecords', value: maxPair[0], onCommit: maxPair[1],
          min: MIN_MAX_RECORDS, max: MAX_MAX_RECORDS,
        }),
        el(StorageRow, {
          t: t, field: 'backfillTimeoutMs', value: budgetPair[0], onCommit: budgetPair[1],
          min: 0, max: MAX_BACKFILL_TIMEOUT_MS,
        }))
    }

    /**
     * One-shot rebuild of records + rollup from the session event logs. A
     * confirmation gate is mandatory: the operation wipes the live store and
     * re-imports, so an accidental click must not fire it. The host backs up
     * first and rolls back on any failure, so the button is safe to press -
     * it just must be deliberate.
     */
    function RebuildSection(props) {
      var t = props.t
      var st = React.useState({ phase: 'idle' })
      var state = st[0]
      var setState = st[1]
      var running = state.phase === 'running'
      var modal = React.useState(false)
      var modalOpen = modal[0]
      var setModal = modal[1]
      // A generous 30-minute timeout (a rebuild must finish, not resume), and
      // one fewer thread than the machine reports so the UI and the model
      // loop keep a core to themselves.
      var timeout = React.useState(1800000)
      var concurrency = React.useState(Math.max(1, (navigator.hardwareConcurrency || 4) - 1))

      function confirm() {
        if (running) return
        setModal(false)
        setState({ phase: 'running' })
        callBill({ action: 'rebuild', timeoutMs: Number(timeout[0]) || 0, concurrency: Number(concurrency[0]) || 1 }).then(function (res) {
          if (res && res.ok) {
            setState({ phase: 'done', records: res.records, totalUsd: res.totalUsd })
          } else {
            setState({ phase: 'error', message: (res && res.error) || 'failed' })
          }
        }, function (err) {
          setState({ phase: 'error', message: (err && err.message) || String(err) })
        })
      }

      var status = null
      if (state.phase === 'running') {
        status = el('div', { style: { ...surfaceHint, color: T.label3 } }, t('rebuild.running'))
      } else if (state.phase === 'done') {
        var done = t('rebuild.done')
          .replace('%n', String(state.records || 0))
          .replace('%u', (Number(state.totalUsd) || 0).toFixed(2))
        status = el('div', { style: { ...surfaceHint, color: T.label } }, done)
      } else if (state.phase === 'error') {
        status = el('div', { style: { ...surfaceHint, color: T.error } }, t('rebuild.failed') + state.message)
      }

      return el('div', { style: section },
        el('div', { style: panelT }, t('rebuild.title')),
        el('div', { style: panelSub }, t('rebuild.desc')),
        el('div', { style: surfaceRow },
          el('div', { style: { minWidth: 0 } },
            el('div', { style: surfaceName }, t('rebuild.button')),
            el('div', { style: surfaceHint }, t('rebuild.button.desc'))),
          el('button', {
            type: 'button',
            className: 'dshbill-btn',
            disabled: running,
            onClick: function () { if (!running) setModal(true) },
          }, running ? t('rebuild.running') : t('rebuild.button'))),
        status,
        modalOpen ? el('div', {
          style: {
          ...
          onClick: function (e) { if (e.target === e.currentTarget && !running) setModal(false) },
        },
          el('div', {
            style: {
              width: '100%', maxWidth: 460, boxSizing: 'border-box',
              background: T.layer2, borderRadius: 12, padding: 20,
              boxShadow: '0 8px 32px rgba(0,0,0,.24)',
            },
          },
            el('div', { style: panelT }, t('rebuild.modal.title')),
            el('div', { style: { ...panelSub, whiteSpace: 'normal' } }, t('rebuild.modal.desc')),
            el('div', { style: surfaceRow },
              el('div', { style: { minWidth: 0 } },
                el('div', { style: surfaceName }, t('rebuild.modal.timeout')),
                el('div', { style: surfaceHint }, t('rebuild.modal.timeout.desc'))),
              el('input', {
                type: 'number', min: 0, step: 1000, className: 'dshbill-field',
                value: timeout[0], onChange: function (e) { timeout[1](Number(e.target.value)) },
                style: surfaceControl, disabled: running,
              })),
            el('div', { style: surfaceRow },
              el('div', { style: { minWidth: 0 } },
                el('div', { style: surfaceName }, t('rebuild.modal.concurrency')),
                el('div', { style: surfaceHint }, t('rebuild.modal.concurrency.desc'))),
              el('input', {
                type: 'number', min: 1, max: 128, step: 1, className: 'dshbill-field',
                value: concurrency[0], onChange: function (e) { concurrency[1](Number(e.target.value)) },
                style: surfaceControl, disabled: running,
              })),
            el('div', { style: { display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 } },
              el('button', {
                type: 'button', className: 'dshbill-btn dshbill-btn-quiet',
                disabled: running, onClick: function () { setModal(false) },
              }, t('rebuild.modal.cancel')),
              el('button', {
                type: 'button', className: 'dshbill-btn',
                disabled: running, onClick: confirm,
              }, t('rebuild.modal.confirm'))))) : null)
    }
