...
/** Ring cap bounds, enforced by the config schema and the prefs store alike. */
const MIN_MAX_RECORDS = 10
const MAX_MAX_RECORDS = 1000000

/** Largest delay setTimeout accepts (beyond it the timer fires immediately). */
const MAX_BACKFILL_TIMEOUT_MS = 2147483647
...
const DEFAULT_PREFS = {
  ...
  showView: true,
  showSidebar: true,
  // Ring size and the boot-time import budget, settable from the settings
  // page. The plugin config still pins the boot default; a value the page
  // wrote to prefs.json wins afterwards.
  maxRecords: DEFAULT_MAX_RECORDS,
  backfillTimeoutMs: DEFAULT_BACKFILL_TIMEOUT_MS,
}

/** Round and clamp a finite number into [min, max]; anything else → fallback. */
...
function normalizePrefs(raw) {
  ...
  return {
    ...
    // Numeric sizing knobs are clamped into their supported ranges; garbage
    // falls back to the boot defaults.
    maxRecords: finiteClamp(Number(input.maxRecords), MIN_MAX_RECORDS, MAX_MAX_RECORDS, DEFAULT_MAX_RECORDS),
    backfillTimeoutMs: finiteClamp(Number(input.backfillTimeoutMs), 0, MAX_BACKFILL_TIMEOUT_MS, DEFAULT_BACKFILL_TIMEOUT_MS),
  }
}
...
export const Config = {
  '~standard': {
    ...
    validate(input) {
      ...
      else if (typeof maxRecords !== 'number' || !Number.isInteger(maxRecords) || maxRecords < MIN_MAX_RECORDS || maxRecords > MAX_MAX_RECORDS) {
        issues.push(issue('expected an integer between 10 and 1000000', 'maxRecords'))
      ...
      if (backfillTimeoutMs === undefined) value.backfillTimeoutMs = DEFAULT_BACKFILL_TIMEOUT_MS
        ...
        issues.push(issue('expected a non-negative integer (0 disables the import)', 'backfillTimeoutMs'))
...
export default {
   ...
   * than trusted.
   */
  // Live sizing knobs. The plugin config pins the boot default; the settings
  // page (prefs.json) overrides them at load time and on every write, so a UI
  // change takes effect without a restart (the ring evicts on the next push).
  let maxRecords = cfg.maxRecords ?? DEFAULT_MAX_RECORDS
  /** Total budget for the boot-time history import; `backfillTimeoutMs` config. */
  let backfillTimeoutMs = cfg.backfillTimeoutMs ?? DEFAULT_BACKFILL_TIMEOUT_MS
   ...
   */
  const emptyRollup = () => ({ ...emptyFold(), byDay: {}, attr: {}, fileSkip: 0, knownSessions: [] })
  ...
  function foldIntoRollup(rec) {
    ...
    // Remember that this session has reached the rollup, so a later boot's
    // backfill pass cannot re-import it once the ring has forgotten it.
    // `bySession` indexes only the live ring; once every record of a session
    // is evicted it drops out, and without this marker the import would pull
    // the same calls in again — folding their cost a second time.
    if (rec.sessionId && !rollup.knownSessions.includes(rec.sessionId)) {
      rollup.knownSessions.push(rec.sessionId)
    }
  }
  ...
  async function loadPrefs() {
    try {
      ...
      if (text !== null) {
        ...
        // The sizing knobs the settings page wrote win over the plugin config;
        // keys the document never carried keep the config's boot values.
        if (Number.isInteger(raw.maxRecords)) maxRecords = prefs.maxRecords
        if (Number.isInteger(raw.backfillTimeoutMs)) backfillTimeoutMs = prefs.backfillTimeoutMs
      } else {
        prefs = { ...DEFAULT_PREFS, maxRecords, backfillTimeoutMs }
      }
    } catch {
  ...
  async function writePrefs(patch) {
    ...
    await withFileLock(file, async () => {
      ...
      // A settings-page change applies immediately: the ring evicts to the new
      // cap on its next push, and the import budget gates the next boot's pass.
      maxRecords = prefs.maxRecords
      backfillTimeoutMs = prefs.backfillTimeoutMs
    })
    return prefs
  }
   ...
   *
   * Runs once at startup (or not at all: `backfillTimeoutMs: 0` disables the
   * import), as soon as capture is live — it does NOT wait for the pricing
   * catalogue, because a cold fetch can hang for tens of seconds or fail
   * offline, and that must not block importing history. An imported record is
   * priced with whatever the catalogue has right now (built-in overrides, a
   * warm file cache, or nothing → usd:null unpriced), and repriceLogRecords
   * tops up the still-unpriced log rows once the catalogue lands. A session
   * that already has ANY record is skipped wholesale, which keeps the pass
   * idempotent across restarts (see the header comment above for the
   * undercounting trade-off).
   *
   ...
   */
  async function backfillFromLog({ budgetMs, concurrency } = {}) {
    // 0 disables the boot-time history import entirely (settings page). The
    // rebuild button passes an override budget so a full re-import is not
    // gated by the (normally small) boot setting.
    const budget = Number.isFinite(budgetMs) ? budgetMs : backfillTimeoutMs
    if (budget === 0) return
    ...
    try {
      ...
      for (const entry of Array.isArray(listed) ? listed : []) {
        ...
        if (id && !bySession.has(id) && !rollup.knownSessions.includes(id)) ids.push(id)
      }
      ...
      const workers = Math.max(1, Number.isFinite(concurrency) ? Math.floor(concurrency) : 4)
      let cursor = 0
      const worker = async () => {
        for (;;) {
          ...
          try {
            ...
            // A forked (seeded) session opens with the parent's events as an
            // inherited prefix — `inheritedEventCount` of them. Those calls
            // already belong to the parent (its own backfill counts them), so
            // importing them here would bill the same calls twice: once under
            // the parent and once under this child. Drop the prefix.
            const inherited = Number.isInteger(obs?.inheritedEventCount) ? obs.inheritedEventCount : 0
            const ownEvents = inherited > 0 ? (obs?.events ?? []).slice(inherited) : (obs?.events ?? [])
            try {
              for (const sample of samplesFromEvents(ownEvents)) {
                const rec = priceRecord({
                  seq: recordSeq++,
                  time: sample.time,
                  sessionId: id,
      ...
      rebuildSessionIndex()
      persist()
      return { imported, aborted: controller.signal.aborted }
    } catch (error) {
      if (controller.signal.aborted) {
        console.warn('dsh-bill: session backfill stopped at the budget —', error?.message ?? error)
      } else {
  ...
  /**
   * Wipe the ring and the rollup and re-import every retained session from
   * its event log — the precise remedy for the inherited-prefix
   * double-count the pre-fix backfill wrote into both. The settings page
   * calls this behind a confirmation; it backs up first and restores the
   * in-memory state on any failure (budget exhausted, session-query missing,
   * no sessions), so a half-finished rebuild never loses data.
   *
   * `loading` is held true throughout: backfill's own persist() and the
   * eviction rollup write are no-ops while it is, so the on-disk files are
   * touched only by the single persist() at the very end.
   */
  async function rebuildFromLog({ timeoutMs, concurrency } = {}) {
    const ts = new Date().toISOString().replace(/[:.]/g, '-')
    const recFile = recordsFile()
    const rollFile = rollupFile()
    const recText = await readFileOrNull(recFile)
    if (recText !== null) await writeFileAtomic(recFile + '.bak-' + ts, recText)
    const rollText = await readFileOrNull(rollFile)
    if (rollText !== null) await writeFileAtomic(rollFile + '.bak-' + ts, rollText)
    // Snapshot the live state so a failed rebuild rolls back in memory. The
    // files were never written (loading suppresses persist), so restoring
    // memory alone is enough to be back where we started.
    const snap = {
      records: records.slice(),
      rollup: JSON.parse(JSON.stringify(rollup)),
      bySession: new Map(bySession),
      recordSeq, persistedCount, persistRewrite, ringVersion,
    }
    loading = true
    records = []
    bySession.clear()
    ...
    persistRewrite = true
    rollup = emptyRollup()
    // The empty ring and empty knownSessions make every retained session
    // eligible; the guard in backfillFromLog passes for all of them. A
    // rebuild must finish, not resume, so the budget is generous and
    // independent of the (normally small) boot setting.
    const result = await backfillFromLog({ budgetMs: timeoutMs || Math.max(backfillTimeoutMs, 1800000), concurrency })
    const ok = result && result.imported > 0 && !result.aborted
    if (!ok) {
      records = snap.records
      rollup = snap.rollup
      bySession.clear()
      for (const [k, v] of snap.bySession) bySession.set(k, v)
      recordSeq = snap.recordSeq
      persistedCount = snap.persistedCount
      persistRewrite = snap.persistRewrite
      ringVersion = snap.ringVersion + 1
      dashboardCache.clear()
      allTimeCache = null
      loading = false
      return {
        ok: false,
        error: result && result.aborted
          ? 'rebuild aborted: import budget exhausted; nothing changed'
          : 'rebuild produced no records (session logs unavailable or empty); nothing changed',
      }
    }
    loading = false
    persist()
    ...
    return { ok: true, records: records.length, totalCalls: total.calls, totalUsd: total.totalUsd, backedUp: ts }
  }
  ...
  async function handleAction(body) {
    ...
    if (action === 'rebuild') {
      try {
        return { ...base, ...(await rebuildFromLog({ timeoutMs: body?.timeoutMs, concurrency: body?.concurrency })) }
      } catch (error) {
