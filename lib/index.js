...
export const Config = {
  '~standard': {
    ...
    validate(input) {
      ...
      if (input === undefined || input === null) return { value: { maxRecords: DEFAULT_MAX_RECORDS, backfillTimeoutMs: DEFAULT_BACKFILL_TIMEOUT_MS } }
...
export default {
   ...
   * import), after capture is live and the pricing catalogue is warm. A
   * session that already has ANY record is skipped wholesale, which keeps the
   * pass idempotent across restarts (see the header comment above for the
