...
export function shellProgram(argumentsJson) {
  ...
  // PowerShell assignment (a line opening with a $-prefixed token like
  // $env:PATH or $base): there is no program to attribute to, and env-var
  // names are case-insensitive, so keying to $env:PATH vs $env:Path would
  // split one spend across two rows. Fall back to the tool name.
