...
`maxRecords` (the in-memory ring buffer size, default 20000, range 10–1,000,000) and `backfillTimeoutMs` (the total budget for the boot-time history import, default 60000; `0` disables the import) are plugin config **and** settable from the **Cost** page — the page's value wins afterwards. `agentTool` (register `bill_stats`, default `true`) and `priceOverrides` are plugin config only and are validated at startup — a mistyped field is reported by name rather than leaving the report quietly empty. `~/.dsh/profiles/web/cordis.patch.yml` is only needed when you want to override a price:
...
| Backfill | lists the session log once, imports sessions it never recorded with a cancellable budget, deduplicating on `turn:step`; a zero budget disables it |
