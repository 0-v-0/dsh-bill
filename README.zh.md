...
`maxRecords`(内存环形缓冲条数,默认 20000,范围 10–1,000,000)与 `backfillTimeoutMs`(启动时历史导入的总预算,默认 60000 毫秒;0 表示禁用导入)既是插件配置,也能在「费用统计」设置页里改,设置页写入的值优先生效;`agentTool`(是否注册 `bill_stats`,默认 `true`)与 `priceOverrides` 走插件配置,会在启动时校验 —— 写错的字段会指名报错,而不是让报告静静地空掉。`~/.dsh/profiles/web/cordis.patch.yml` 只在需要覆盖价格时才用得上:
...
| 回填 | 会话日志只列一次,以可取消的预算导入未记录的会话,按 `turn:step` 去重;预算为 0 时禁用 |
